	...
	view.andry-order-list
		view.u-tabs-box
			u-tabs-swiper(
				...
				:is-scroll="false", 
				swiperWidth="750")
		swiper.swiper-box(:current="swiperCurrent" @transition="transition" @animationfinish="animationfinish")
						...
						view.order(
							...
							view.top
								view.left
									...
									view.store {{ item.store }}
									u-icon(name="arrow-right", color="rgb(203,203,203)", :size="26")
								view.right {{ item.deal }}
							view.item(v-for="(item, index) in item.goodsList", :key="index")
								view.left
									image(:src="item.goodsUrl", mode="aspectFill")
								view.content
									view.title.u-line-2 {{ item.title }}
								view.right
									view.price
										text.unit ￥
										text.money {{ priceInt(item.price) }}
										text.decimal .{{ priceDecimal(item.price) }}
									view.number x{{ item.number }}
							view.bottom
								view.more
									u-icon(name="more-dot-fill", color="rgb(203,203,203)")
								view.logistics.btn 查看物流
								view.exchange.btn 卖了换钱
								view.evaluate.btn 评价
						u-loadmore.loadmore(:status="loadStatus[0]", bgColor="#f2f2f2")
			swiper-item.swiper-item
				scroll-view(scroll-y style="height: 100%;width: 100%;", @scrolltolower="reachBottom")
					view.page-box
						view.order(
							v-for="(item, index) in  orderList[1]", 
							:key="item.id",
							@click="onOrderDetail(item)")
							view.top
								view.left
							...
							view.bottom
								...
								view.exchange.btn 卖了换钱
								view.evaluate.btn 评价
						u-loadmore.loadmore(:status="loadStatus[1]", bgColor="#f2f2f2")
			swiper-item.swiper-item
					...
					view.page-box
						view
							view.centre
								...
								view.explain
									view 您还没有相关的订单
									view.tips 可以去看看有那些想买的
								view.btn 随便逛逛
			swiper-item.swiper-item
				scroll-view(scroll-y style="height: 100%;width: 100%;", @scrolltolower="reachBottom")
					view.page-box
						view.order(
							v-for="(item, index) in  orderList[2]", 
							:key="item.id",
							@click="onOrderDetail(item)")
							view.top
								view.left
							...
							view.bottom
								...
								view.exchange.btn 卖了换钱
								view.evaluate.btn 评价
						u-loadmore.loadmore(:status="loadStatus[2]", bgColor="#f2f2f2")
</template>
...
export default class OrderList extends Vue {
	...
	getOrderList(idx) {
		for (let i = 0; i < 5; i++) {
			const index: any = (uni as any).$u.random(
				0,
				this.dataList.length - 1
			);
			const data: any = JSON.parse(JSON.stringify(this.dataList[index]));
			data.id = (uni as any).$u.guid();
	...
	transition({ detail: { dx } }) {
		// (this.$refs.tabs as any).setDx(dx);
	}
